...
class CaptureEngine {
    ...
    async captureFullPageFallback(tabId) {
        try {
            ...
            if (sections > 20) {
                throw new Error(`Cette page est trop longue pour être capturée (${sections} sections). Maximum: 20 sections.`);
            }

            if (totalHeight > 50000) {
                throw new Error(`Cette page est trop haute (${totalHeight}px). Hauteur maximum: 50000px.`);
            }
    ...
    async attachDebugger(tabId) {
        ...
        try {
            ...
            if (error.message.includes('Another debugger')) {
                throw new Error('DevTools est déjà ouvert sur cet onglet. Veuillez le fermer et réessayer.');
            }
            if (error.message.includes('No target')) {
                throw new Error('Impossible d\'accéder à cette page. Veuillez rafraîchir et réessayer.');
            }
            throw error;
        }
    }
